...
- **Exercise images come from a committed manifest, not a guessed URL.**
  `src/data/exerciseMediaManifest.ts` is generated by
  `npm run seo:resolve-exercise-media`, which probes the `exercise-media` bucket
  for the extension that actually exists and records it with the image's
  intrinsic dimensions. The library stores `{id}/sequence.jpeg` by convention and
  the app retries `.png`/`.jpg` in the browser; a static page cannot retry, so
  guessing would ship broken images. Builds read the committed file and never
  call Supabase. An exercise missing from the manifest renders no image — that is
  intended.
