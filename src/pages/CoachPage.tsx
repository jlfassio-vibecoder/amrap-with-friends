...
import { CoachTabPanel } from '@/components/coach/CoachTabPanel';
...
export default function CoachPage() {
  ...
  const tabs = COACH_TABS.map((tab) =>
    tab.key === 'applications' && showApplicationsNewBadge ? { ...tab, badge: 'New' } : tab
  );
  ...
  return (
    <main className="min-h-screen bg-page">
      ...
      <div className="mx-auto max-w-6xl space-y-8 px-6 pb-10 pt-0 lg:px-8 lg:py-10">
        ...
        <CoachTabPanel tab="funnels" activeTab={activeTab}>
            ...
            <>
              <section className="space-y-3">
                ...
                <CoachStatGrid
                  ...
                  stats={[
                    ...
                    {
                      label: 'Missions created (30d)',
                      value: dashboard.topStrip.missionsCreated30d,
                    },
                    {
                      label: 'Missions finished (7d)',
                      value: dashboard.topStrip.missionsFinished7d,
                    },
                    {
                    ...
                    {
                      label: 'Live missions created',
                      value: dashboard.topStrip.liveMissionsCreated,
                    },
                    {
                ...
                <p className="text-xs text-secondary">
                  These tiles name their own window and do not follow the picker; every section
                  below does.
                </p>
                {guestBrowsersOpen ? (
              ...
              <section className="space-y-3">
                ...
                <div className="card space-y-2 p-4">
                  ...
                  />
                  <p className="text-xs text-secondary">
                    Google sign-up completions are not observable — the OAuth redirect leaves the
                    app, and the sign-in that comes back carries no link to the attempt that started
                    it.
                  </p>
                </div>
              </section>
              ...
              <section className="space-y-3">
                ...
                <div className="card space-y-3 p-4">
                  ...
                  <p className="text-xs text-secondary">
                    ...
                    registered users only — an anon id is per-browser, so guest retention would
                    mostly measure cookie lifetime. Not affected by the window picker: a cohort
                    curve has to look back further than the window to fill in.
                  </p>
                </div>
              </section>
        ...
        <CoachTabPanel tab="content-acquisition" activeTab={activeTab}>
          {loading ? <p className="text-sm text-secondary">Loading…</p> : null}
          {error ? <p className="text-error text-sm">{error}</p> : null}
          {dashboard ? (
            <>
              <div className="flex flex-wrap items-center justify-end gap-3">
                <CoachWindowPicker value={reportWindow} onChange={handleWindowChange} />
              </div>
              ...
              <section className="space-y-3">
                ...
                <div className="card space-y-3 p-4">
                  ...
                  <p className="text-xs text-secondary">
                    ...
                    referral, so the site cannot take credit for its own traffic. Only browsers seen
                    since this shipped appear here.
                  </p>
                </div>
              </section>
              ...
              <section className="space-y-3">
                ...
                <div className="card space-y-3 p-4">
                  ...
                  <p className="text-xs text-secondary">
                    ...
                    someone can scroll to the bottom hunting for a link without reading a word. A
                    page that fits on one screen counts as fully scrolled.
                  </p>
                </div>
              </section>
              ...
              <section className="space-y-3">
                ...
                <div className="card space-y-3 p-4">
                  ...
                  />
                  <p className="text-xs text-secondary">
                    Our own hostname here is a stale internal link to fix; another site is a
                    redirect to add. Reported without a browser id and without touching storage, so
                    it needs no consent anywhere — a 404 is a fact about a URL, not about a person.
                  </p>
                </div>
              </section>
              ...
              <section className="space-y-3">
                ...
                <div className="card space-y-3 p-4">
                  ...
                  />
                  <p className="text-xs text-secondary">
                    A named placement comes from a <code>data-cta</code> on the link; everything
                    else is named for the landmark it sits in — nav, header, footer or inline — so
                    nothing had to be annotated to be countable. Rows are per source page, because
                    the same placement means different things on different pages: inline links in a
                    guide are the guide's argument, inline links on the home page are mostly
                    navigation.
                  </p>
                </div>
              </section>
              ...
              <section className="space-y-3">
                ...
                <div className="card space-y-3 p-4">
                  ...
                  {toolLiftIsMeaningful(dashboard.toolConversion, 'timer_completed') ? (
                    ...
                    <p className="text-sm text-secondary">
                      Not enough visitors in both groups yet — the lift is only reported once
                      readers and tool users each clear 20 browsers.
                    </p>
                  )}
                  <p className="text-xs text-secondary">
                    Cohorts overlap except "read only": someone who finished a run and also scored
                    their splits is counted in both, because each row answers "of the browsers who
                    did this, how many went on to train". Visitors who declined consent have no
                    identifier and are excluded here; their pageviews still count above.
                  </p>
                </div>
              </section>
              ...
              <section className="space-y-3">
                ...
                <div className="card space-y-4 p-4">
                  <CoachDataTable
                    ...
                    rowKey={(row) =>
                      `${row.templateId}-${row.durationMinutes}-${row.intensityTier}`
                    }
                    ...
                    columns={[
                      ...
                      {
                        header: 'Completed',
                        render: (row) => row.missionsCompleted,
                        align: 'right',
                      },
                      {
                        header: 'Completion %',
        ...
        <CoachTabPanel tab="reliability" activeTab={activeTab}>
          {loading ? <p className="text-sm text-secondary">Loading…</p> : null}
          {error ? <p className="text-error text-sm">{error}</p> : null}
          {dashboard ? (
            <>
                  ...
                  <div className="card space-y-2 p-4">
                    ...
                    <CoachDataTable
                      ...
                      columns={[
                        ...
                        {
                          header: 'Error %',
                          render: (row) => pct(row.errorRatePct),
                          align: 'right',
                        },
                        {
                          header: 'p50 ms',
        ...
        <CoachTabPanel tab="explore" activeTab={activeTab}>
          <section className="space-y-3">
