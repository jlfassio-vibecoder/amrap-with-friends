...
import { formatPlusElapsed } from '@/lib/mission/rallyPointCountdown';
...
export function ArmedRallyPointControls({
  ...
  async function abortClock() {
    ...
    if (!hostToken) {
      setError('Host credentials are missing. Reopen the mission as host.');
      return;
    }
    setBusy(true);
    setError(null);
    try {
      const result = await cancelRallyPointCountdown({ missionId, hostToken });
      if (result.error) {
