...
describe('buildRallyInviteUrl', () => {
  it('builds /join?m= with origin, mission id, and default female card', () => {
    expect(buildRallyInviteUrl(MISSION_ID, 'https://amrap.example')).toBe(
      `https://amrap.example/join?m=${MISSION_ID}&card=f`
    );
  });

  it('bakes a male card when requested', () => {
    expect(buildRallyInviteUrl(MISSION_ID, 'https://amrap.example', 'm')).toBe(
      `https://amrap.example/join?m=${MISSION_ID}&card=m`
    );
  });
});
...
describe('CopyInviteLink', () => {
  ...
  it('copies the rally URL and flashes LINK COPIED', async () => {
    ...
    render(<CopyInviteLink missionId={MISSION_ID} />);
