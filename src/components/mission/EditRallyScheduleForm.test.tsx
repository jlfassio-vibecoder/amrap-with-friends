...
const updateMissionScheduledAtMock = vi.fn();
const TEST_TIME_ZONE = 'America/Los_Angeles';

vi.mock('@/lib/api/missions', () => ({
  updateMissionScheduledAt: (...args: unknown[]) => updateMissionScheduledAtMock(...args),
}));
...
afterEach(() => {
  cleanup();
  updateMissionScheduledAtMock.mockReset();
  vi.restoreAllMocks();
});
...
describe('EditRallyScheduleForm', () => {
  it('prefills day and time from scheduledAt', () => {
    ...
    render(<EditRallyScheduleForm missionId={MISSION_ID} scheduledAt="2026-08-25T05:00:00.000Z" />);
  ...
  it('shows validation error for invalid rally time', async () => {
    ...
    expect(updateMissionScheduledAtMock).not.toHaveBeenCalled();

    vi.useRealTimers();
  });

  it('calls updateMissionScheduledAt and onSaved on success', async () => {
    vi.setSystemTime(new Date('2026-08-25T04:00:00.000Z'));
    const onSaved = vi.fn();
    updateMissionScheduledAtMock.mockResolvedValue({
    ...
    await waitFor(() => {
      expect(updateMissionScheduledAtMock).toHaveBeenCalledWith({
        missionId: MISSION_ID,
