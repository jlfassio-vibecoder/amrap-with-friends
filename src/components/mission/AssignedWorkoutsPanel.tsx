 ...
 *
 * my_missions() answers "what did I train?"; this is the other half — something
 * waiting to be trained. Starting one creates an ordinary mission from the
...
export function AssignedWorkoutsPanel() {
  ...
  async function handleStart(entry: AssignedWorkout) {
    ...
    try {
      ...
      // Best effort: the mission is real either way, and leaving the row
      // pending is better than blocking the athlete from training.
      // createMission already persists identity (nickname, tokens, claim).
      await startAssignedWorkout(entry.assignedWorkoutId, created.data.missionId);
      navigate(`/mission/${created.data.missionId}`);
    } finally {
      setBusyId(null);
    }
  }
    ...
    <section className="card space-y-4 p-6">
      <div>
        ...
        <p className="text-sm text-secondary">
          Workouts your squad picked out for you. Starting one runs it like any other mission.
        </p>
      </div>
