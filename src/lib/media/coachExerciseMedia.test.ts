...
import { getCoachExerciseMediaUrl, uploadCoachExerciseImage } from './coachExerciseMedia';
...
describe('getCoachExerciseMediaUrl', () => {
  ...
  it('resolves a public url for a relative path', () => {
    expect(getCoachExerciseMediaUrl('coach-id/exercise-id.jpg')).toBe(
      'https://example.test/public/path.jpg'
    );
  });
});

describe('uploadCoachExerciseImage', () => {
  it('rejects an unsupported mime type', async () => {
    const file = makeFile('clip.gif', 'image/gif', 1024);
    const result = await uploadCoachExerciseImage('coach-1', 'ex-1', file);
  ...
  it('uploads a valid image and returns the owner/exercise-scoped path', async () => {
    uploadMock.mockResolvedValue({ error: null });
    const file = makeFile('photo.jpeg', 'image/jpeg', 1024);

    const result = await uploadCoachExerciseImage('coach-1', 'ex-1', file);

    expect(uploadMock).toHaveBeenCalledWith('coach-1/ex-1.jpeg', file, {
      upsert: true,
      contentType: 'image/jpeg',
    });
    expect(result.error).toBeNull();
    expect(result.path).toBe('coach-1/ex-1.jpeg');
  });
