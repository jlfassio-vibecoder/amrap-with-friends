...
import type {
  ...
  UpdateMissionStateInput,
  UpdateMissionStateResult,
} from '@/lib/missionSync/types';

export type MissionSyncApiError = {
  message: string;
};
...
function isLiveMissionPhase(value: string): value is LiveMissionPhase {
...
function mapRpcError(message: string | undefined): string {
  ...
  if (message.includes('Invalid mission state')) {
    return 'Invalid mission state.';
  }
...
export async function updateMissionState(input: UpdateMissionStateInput): Promise<{
  data: UpdateMissionStateResult | null;
  error: MissionSyncApiError | null;
}> {
  const { data, error } = await callRpc('update_mission_state', {
...
export async function logRound(
  input: LogRoundInput
): Promise<{ data: LogRoundResult | null; error: MissionSyncApiError | null }> {
...
function mapRallyPointCountdownReason(reason: string): string {
  switch (reason) {
    case 'invalid_host_token':
      return 'Host credentials are invalid. Reopen the mission as host.';
    case 'mission_not_waiting':
...
export async function cancelRallyPointCountdown(input: {
  ...
  track('rally_point_countdown_canceled', {}, { missionId: input.missionId });
