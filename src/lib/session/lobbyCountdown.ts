export const LOBBY_COUNTDOWN_MAX_SECONDS = 600;

export function remainingLobbyCountdownSec(
...
/** Seconds since the lobby countdown ended (0 while still before ends_at). */
export function elapsedPastLobbyCountdownSec(
  endsAtIso: string | null | undefined,
  nowMs: number
): number | null {
  if (!endsAtIso) {
...
export function formatTMinus(remainingSec: number): string {
  ...
  return `T-MINUS ${minutes.toString().padStart(2, '0')}:${seconds
    .toString()
    .padStart(2, '0')}`;
}
