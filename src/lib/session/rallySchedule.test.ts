...
describe('rallySchedule', () => {
  ...
  it('allows today and tomorrow in the host timezone and rejects past or later days', () => {
    ...
    expect(
      isRallyTimeAllowed('2026-08-25T05:00:00.000Z', TZ, now)
    ).toBe(true);
    expect(
      isRallyTimeAllowed('2026-08-26T06:59:00.000Z', TZ, now)
    ).toBe(true);
    expect(
      isRallyTimeAllowed('2026-08-25T03:00:00.000Z', TZ, now)
    ).toBe(false);
    expect(
      isRallyTimeAllowed('2026-08-27T04:00:00.000Z', TZ, now)
    ).toBe(false);
  });
