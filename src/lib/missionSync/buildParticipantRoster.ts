...
export function compareAbsoluteRoster(
  a: RosterMergeEntry,
  b: RosterMergeEntry,
  phase: LiveMissionPhase = 'finished'
): number {
  if (phase !== 'finished') {
...
export function buildParticipantRoster(
  ...
  const filtered =
    sortMode === 'discipline' ? entries.filter((entry) => entry.pvi !== null) : entries;

  const sorted = [...filtered].sort((a, b) =>
    sortMode === 'discipline' ? compareDisciplineRoster(a, b) : compareAbsoluteRoster(a, b, phase)
  );
