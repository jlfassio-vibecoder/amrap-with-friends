...
/** Setup length used when a featured mission is manually started (host Start). */
export const FEATURED_SETUP_DURATION_SEC = DEFAULT_SETUP_DURATION_SEC;

export interface FeaturedMissionClock {
 ...
 */
export function computeFeaturedMissionClock(input: {
  scheduledAtMs: number;
  durationMinutes: number;
  nowMs: number;
  setupDurationSec?: number;
}): FeaturedMissionClock {
  ...
  if (input.nowMs < setupEndsAtMs) {
    const timeLeftSec = Math.max(1, Math.ceil((setupEndsAtMs - input.nowMs) / 1000));
    return {
      phase: 'setup',
      timeLeftSec,
      workStartedAtMs: null,
