...
describe('missionChannelUtils', () => {
  it('parseMissionRow validates mission shape', () => {
    const row = parseMissionRow({
      id: MISSION_ID,
      duration_minutes: 15,
  ...
  it('parseMissionRow keeps is_featured when present', () => {
    const row = parseMissionRow({
      id: MISSION_ID,
      duration_minutes: 15,
      workout: [{ name: 'Burpees' }],
  ...
  it('parseMissionRow keeps rally_point_countdown_ends_at when present', () => {
    const row = parseMissionRow({
      id: MISSION_ID,
      duration_minutes: 15,
      workout: [{ name: 'Burpees' }],
