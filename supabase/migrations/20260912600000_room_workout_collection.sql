...
--
-- The workout jsonb is returned, not just the template id. CLAUDE.md's rule:
-- a mission carries its own copy so an edited template cannot retroactively
-- change a recorded result. An athlete launching from the collection must get
-- the workout the room ran, not whatever the library says today.
--
