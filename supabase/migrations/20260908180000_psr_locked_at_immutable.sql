-- Immutable score-lock timestamp for HUD / load windows.
--
-- lastLockedAt (and week / attrition / overtraining windows) used
-- participant_segment_results.updated_at as a proxy for lock time. That column
-- moves on any later write — including admin score repairs — so the HUD clock
-- can jump to ~00:00 without a new lock. locked_at is stamped once when
-- score_breakdown first becomes non-null and never overwritten.
...
GRANT SELECT (locked_at) ON public.participant_segment_results TO anon, authenticated;
...
DROP TRIGGER IF EXISTS participant_segment_results_set_locked_at
  ON public.participant_segment_results;

CREATE TRIGGER participant_segment_results_set_locked_at
  BEFORE INSERT OR UPDATE ON public.participant_segment_results
  FOR EACH ROW
  EXECUTE FUNCTION public.participant_segment_results_set_locked_at();

CREATE INDEX IF NOT EXISTS idx_participant_segment_results_locked_at
  ON public.participant_segment_results (participant_id, locked_at DESC)
  WHERE score_breakdown IS NOT NULL AND locked_at IS NOT NULL;

-- Overtraining windows: same lock clock as the HUD.
...
BEGIN
  ...
  WHERE p.user_id = p_user_id
    AND psr.score_breakdown IS NOT NULL
    AND coalesce(psr.locked_at, psr.updated_at) >= now() - interval '28 days';
    ...
    SELECT
      EXISTS (
        ...
        WHERE p.user_id = p_user_id
          ...
          AND coalesce(psr.locked_at, psr.updated_at) >= ((v_local_today - v_day_offset)::timestamp AT TIME ZONE v_tz)
          AND coalesce(psr.locked_at, psr.updated_at) < ((v_local_today - v_day_offset + 1)::timestamp AT TIME ZONE v_tz)
      )
      OR EXISTS (
        SELECT 1
...
BEGIN
  ...
  SELECT
    coalesce(sum(s.duration_minutes) FILTER (WHERE s.duration_minutes BETWEEN 3 AND 5), 0)::int,
    coalesce(sum(s.duration_minutes) FILTER (WHERE s.duration_minutes BETWEEN 7 AND 10), 0)::int,
    coalesce(sum(s.duration_minutes) FILTER (WHERE s.duration_minutes BETWEEN 12 AND 15), 0)::int,
    coalesce(sum(s.duration_minutes) FILTER (WHERE s.duration_minutes >= 18), 0)::int,
    coalesce(
      sum(s.duration_minutes) FILTER (
        WHERE s.duration_minutes NOT BETWEEN 3 AND 5
