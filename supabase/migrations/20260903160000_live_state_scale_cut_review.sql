-- Review fixes for live-state scale cuts (20260903150000 already applied):
-- 1. Serialize send_message rate-limit check/insert with advisory xact lock.
-- 2. Return snapshot_at so quiet missions can advance the client watermark.
-- 3. Include joins at the watermark (joined_at >= p_since).
...
BEGIN
  ...
  -- Serialize per seat so concurrent sends cannot both pass the rate-limit window.
  PERFORM pg_advisory_xact_lock(
    hashtextextended(p_mission_id::text || ':' || p_participant_id::text, 0)
  );
