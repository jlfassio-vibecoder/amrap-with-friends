-- Refuse a null round log at the door.
--
-- p_mission_id and p_participant_id were null-checked; the three integers were
-- only range-checked, and `NULL < 0` is NULL, which an IF treats as false. A
-- null slipped through to the insert and came back as a NOT NULL violation --
-- a 500 where the function had a perfectly good refusal to give.
--
-- Pre-existing, and carried forward unchanged by 20260910600000; found by
-- review on that migration.
...
BEGIN
  ...
  -- Null-checked before the range check, because `NULL < 0` is NULL and an IF
  -- treats that as false: a null index slid past this guard and surfaced much
  -- later as a NOT NULL violation on the insert, which reaches the client as a
  -- 500 instead of the refusal this line is here to give.
  IF p_round_index IS NULL OR p_elapsed_sec_at_round IS NULL OR p_segment_index IS NULL THEN
    RAISE EXCEPTION 'Invalid round log';
  END IF;
