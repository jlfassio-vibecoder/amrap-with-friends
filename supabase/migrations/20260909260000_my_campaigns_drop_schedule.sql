-- my_campaigns stops shipping the schedule.
--
-- 20260909220000 added it so the Benchmarks card could work out which domain
-- each live campaign tests in. benchmark_overview now returns those schedules
-- itself, scoped to live campaigns and fetched only by the surfaces that need
-- them — so my_campaigns was left sending up to sixty occurrence rows per
-- campaign to every caller of My campaigns, and nothing read them.
--
-- Reverted to the 20260909140000 body verbatim. The campaign schedule still
-- ships raw and deriveCampaignRoles still decides; only the door it comes
-- through has changed.
