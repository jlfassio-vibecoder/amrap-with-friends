-- Return the 12 weeks hud_telemetry already walks, instead of reducing each
-- to a single compliant/deficient bit.
--
-- The attrition CTE has always computed real per-week minutes on lock-time
-- boundaries and thrown everything but the boolean away. The HUD's week
-- history panel needs those numbers, so this adds a parallel 'weeks' array
-- carrying minutes, score, PVI average and the missions themselves.
--
-- 'attrition' is deliberately left byte-identical: a client running ahead of
-- this migration keeps its grid, and one running behind ignores the new key.
