-- Tighten pos0 heal: only stamp when the hub active mission matches the
-- queued position-0 workout (duration / workout / template / tier). Hub
-- membership alone can attach an unrelated daisy-chain active mission.
...
BEGIN
  ...
  IF FOUND AND v_pos0.started_mission_id IS NULL THEN
    ...
    IF v_active_mission_id IS NOT NULL
      AND EXISTS (
        ...
        WHERE m.id = v_active_mission_id
          ...
          AND m.duration_minutes = v_pos0.duration_minutes
          AND m.workout = v_pos0.workout
          AND m.template_id IS NOT DISTINCT FROM v_pos0.template_id
          AND m.intensity_tier IS NOT DISTINCT FROM v_pos0.intensity_tier
      )
      AND NOT EXISTS (
        SELECT 1
    ...
    THEN
      -- Copilot suggestion ignored: full read-path chain normalize is out of scope; heal-on-advance is the intended repair.
