-- Wire activation funnel + waiting graveyard into coach_dashboard.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'activationFunnel', (SELECT to_jsonb(v) FROM public.report_activation_funnel(v_since) v),
    'waitingGraveyard', (SELECT to_jsonb(v) FROM public.report_waiting_graveyard(v_since) v),
    'signupFunnel', (
    ...
    'contentPerformance', (
      SELECT coalesce(jsonb_agg(v), '[]'::jsonb)
      FROM public.report_content_performance(v_since, 50) v
    ),
    'toolConversion', (
      SELECT coalesce(jsonb_agg(v), '[]'::jsonb) FROM public.report_tool_conversion(v_since) v
    ),
    'ctaPlacement', (
      SELECT coalesce(jsonb_agg(v), '[]'::jsonb) FROM public.report_cta_placement(v_since, 50) v
    ),
    'contentEngagement', (
      SELECT coalesce(jsonb_agg(v), '[]'::jsonb)
      FROM public.report_content_engagement(v_since, 50) v
    ),
    'notFound', (
      SELECT coalesce(jsonb_agg(v), '[]'::jsonb) FROM public.report_not_found(v_since, 50) v
    )
  );
END;
$function$;
