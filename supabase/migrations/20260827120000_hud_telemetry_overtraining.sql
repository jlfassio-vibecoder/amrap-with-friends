-- Phase B: overtraining load signals on hud_telemetry. Pure aggregation
-- here — the risk-level verdict and warning copy are computed client-side
-- by evaluateOvertrainingRisk() (src/lib/hud/evaluateOvertrainingRisk.ts),
-- mirroring how evaluateLoadImbalance() already works for the domain
-- matrix. Load = duration_minutes * intensity_tier (session-RPE method),
-- combining eligible (claimed + locked) AMRAP sessions with the athlete's
-- own physical_activity_log entries — this is the one place outside
-- activity feeds into HUD math, and it never touches weekMinutes or
-- classification, only this separate overtraining block.
...
BEGIN
  ...
  -- Overtraining load: session-RPE (duration * intensity), rolling windows
  -- from now(), combining eligible AMRAP sessions with physical_activity_log.
    ...
    SELECT
      EXISTS (
        ...
        WHERE p.user_id = v_uid
          ...
          AND psr.updated_at >= ((v_local_today - v_day_offset)::timestamp AT TIME ZONE p_timezone)
          AND psr.updated_at < ((v_local_today - v_day_offset + 1)::timestamp AT TIME ZONE p_timezone)
      )
      OR EXISTS (
        SELECT 1
        ...
        WHERE pal.user_id = v_uid
          ...
          AND pal.occurred_at >= ((v_local_today - v_day_offset)::timestamp AT TIME ZONE p_timezone)
          AND pal.occurred_at < ((v_local_today - v_day_offset + 1)::timestamp AT TIME ZONE p_timezone)
      )
    INTO v_is_hard_day;
