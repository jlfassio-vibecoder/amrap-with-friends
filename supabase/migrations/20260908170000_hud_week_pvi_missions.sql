-- Add this week's per-mission PVI list next to the weekly mean.
--
-- The card can already say the average hides its shape; without the list it
-- cannot name which mission was uneven. Same ISO-week filter and numeric-pvi
-- gate as weekPviAverage so the mean and the rows stay aligned.
...
BEGIN
  ...
  SELECT coalesce(
    jsonb_agg(mission_row ORDER BY locked_at DESC),
    '[]'::jsonb
  )
  ...
  FROM (
    SELECT
      jsonb_build_object(
        ...
        'lockedAt', psr.updated_at
      ) AS mission_row,
      psr.updated_at AS locked_at
    FROM public.participants p
    INNER JOIN public.missions s ON s.id = p.mission_id
    INNER JOIN public.participant_segment_results psr
      ON psr.participant_id = p.id
