-- Score the Long domain by range, not by the exact canonical minute.
--
-- Two filters in hud_telemetry keyed off `duration_minutes = 20`. Once the time
-- cap is selectable within a domain, both go wrong in ways nobody would report
-- as a bug because nothing visible breaks:
--
--   * `marathon_20` is a hard prerequisite for the Special Ops rank. An athlete
--     who ran an 18- or 25-minute Long mission earned no credit toward it and
--     was told nothing. That is the reward ladder failing silently in the
--     direction of taking something away.
--   * The domain-mix sums dropped every non-canonical cap into `other`, so a
--     7-minute Short mission vanished from the chart and the load-imbalance
--     warning nagged about a domain the athlete had actually trained.
--
-- Both now use the ranges from src/lib/timeDomains.ts: 3-5, 7-10, 12-15, 18+.
-- The Long bound is open on top rather than capped at 25 so that coach WODs
-- longer than the selector allows still count as long work, which is what they
-- plainly are.
--
-- Nothing changes for a mission run at 5, 10, 15 or 20 minutes.
